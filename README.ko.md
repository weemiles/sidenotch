...
바로 그 값이다. 링에는 이번 주에 남은 양, 즉 전체 모델 주간 한도를 표시하고, 5시간
한도와 Fable처럼 특정 모델에만 걸리는 한도는 패널 아래쪽에 따로 표시한다. 토큰은 Claude Code가
...
Codex는 5시간·주간·월간 등 여러 창을 보고하는데, **그중 무엇이 `primary`에
들어가는지가 일정하지 않다.** 같은 계정에서도 요청에 따라 달라진다. 그래서
`primary`만 읽으면, 주간 한도가 거의 다 찼는데도 거의 안 쓴 5시간 창이 표시되는
일이 생긴다.

지금은 모든 창을 모아 각각의 최신 값을 유지하고, **가장 빡빡한 창**(곧 막을 창)을
게이지에 표시한다. 나머지는 패널에 함께 나열한다.
...
| 키 | 기본값 | 설명 |
|---|---|---|
| `claudeFiveHourBudgetUSD` | *(자동)* | Claude 게이지가 0%가 되는 지점. 비워두면 이력에서 계산 |
...
따로 설정할 게 없다. 첫 실행 때 **본인 이력에서 자동으로 계산한다.** 앱에 박아둔
숫자는 다른 사람 기기에서 아무 의미가 없기 때문이다.

가장 좋은 신호는 실제로 거부당한 창이다. Claude Code는 한도에 걸리면
`five_hour` 거부를 리셋 시각과 함께 기록하는데, 그 창의 가중 사용량이 곧 그
계정이 바닥나는 지점이다. 그런 창들의 중앙값을 예산으로 쓴다. 게이지 아래
**실측 (5회)** 라고 뜨면 실제 중단 5번이 그 숫자의 근거라는 뜻이다.

한 번도 걸려본 적이 없으면 잴 것이 없으므로, 지금까지 가장 많이 쓴 창을 쓰고
**$X 이상 · 중단 이력 없음** 이라고 표시한다. 한도가 아니라 하한선이다 — 그 창은
거부당하지 않았으니, 이 계정이 최소한 그만큼은 버틴다는 사실만 말해준다. 더 쓸수록
올라가고, 처음 한도에 걸리는 순간 실측값으로 바뀐다. 다음 날을 기다리지 않고 바로
반영한다.

하루에 한 번 다시 계산하며, `claudeFiveHourBudgetUSD`에 숫자를 넣으면 그 값으로
고정된다.

**공용·조직 계정 주의.** Claude 계정을 여러 사람이 함께 쓰면 한도를 깎는 사용량
...
게이지를 그 두 퍼센트로 고정한다(앞이 Claude, 뒤가 Codex). 이 동안은 디스크를
아예 읽지 않으므로 촬영 중에 숫자가 바뀌지 않는다.
